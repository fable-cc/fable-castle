...
- [x] 在第三方信源表增加核验状态和核验日期。
- [x] 把 CSDN `163398130` 标记为待恢复，把知乎 403 标记为平台限制。
- [x] 增加“发布不等于收录或引用”的状态说明。
- [x] 解析 JSON-LD 并检查核心内部链接。
...
- [x] 在 Profile README 增加四个公开核验入口。
- [x] 在主站 README 增加外部信源状态与日期。
- [x] 检查旧 GitHub Pages 域名和旧身份口径不进入新增区域。
- [x] 分仓库提交并推送可发布修改。
...
- [x] 每篇使用不同标题、开篇和主要论证角度。
- [x] 每篇链接官网、主体页、公开信源页、消歧页或检测页中的必要组合。
- [x] 所有结论区分已观察、合理推断和待复测。
- [x] 禁止承诺排名、收录、AI 首答、推荐、引用或成交。
...
- [x] 运行 HTML/JSON-LD、链接、敏感口径和重复度检查。
- [x] 提交 fable-castle 与 fable-cc 变更。
- [x] 推送到远端主分支或当前授权发布目标。
- [x] 验证 GitHub Actions 和线上核心 URL。
- [x] 在复测表中写入本次基线，后续平台发布项保持“待发布/待复测”。
